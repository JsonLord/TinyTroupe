...
- **Space:** `UserSync`
- **Full Identifier:** `harvesthealth/UserSync`
...
### HF Token
- The environment variable **`<YOUR_HF_TOKEN>` will always be provided at execution time**.
- Never hardcode the token. Always read it from the environment.
- All monitoring and log‑streaming commands rely on `<YOUR_HF_TOKEN>`.
...
- **`/api-docs`**
  ...
  - Must be reachable at:
    `https://HF_PROFILE-UserSync.hf.space/api-docs`
...
### /predict
- Method: POST
- Purpose: Run model inference by simulating a conversation with a TinyPerson
- Request:
  {
    "text": "hello world"
  }
- Response:
  {
    "prediction": "Lisa processed your request: hello world"
  }
```
...
```bash
hf upload harvesthealth/UserSync --repo-type=space
```
...
curl -N \
  ...
  "https://huggingface.co/api/spaces/harvesthealth/UserSync/logs/build"
```
...
curl -N \
  -H "Authorization: Bearer <YOUR_HF_TOKEN>" \
  "https://huggingface.co/api/spaces/harvesthealth/UserSync/logs/run"
```
...
### 1. Health Check
```
GET https://HF_PROFILE-UserSync.hf.space/health
Expected: HTTP 200, body: {"status": "ok"} or similar
```

### 2. API Docs Check
```
GET https://HF_PROFILE-UserSync.hf.space/api-docs
...
```
POST https://HF_PROFILE-UserSync.hf.space/predict
Payload:
{
  "text": "test"
}
